...
function smallPill(status: string) {
  ...
  const cls =
    up === "COMPLIANT" ? "border-emerald-200 bg-emerald-50 text-emerald-700"
    : up === "IN_PROGRESS" ? "border-amber-200 bg-amber-50 text-amber-700"
    : up === "NON_COMPLIANT" ? "border-rose-200 bg-rose-50 text-rose-700"
    : up === "NOT_EVALUATED" ? "border-slate-200 bg-slate-50 text-slate-700"
...
function ObligationsGlobalesContent() {
  ...
  async function openDrawer(o: GlobalObligation, opts?: { syncUrl?: boolean }) {
    setSelected(o);
    setDrawerOpen(true);
    if (opts?.syncUrl !== false) setUrlObligationId(o.obligationId);
  ...
  useEffect(() => {
    ...
    if (!oid) { didAutoOpen.current = true; return; }
    const found = obligations.find((o) => String(o.obligationId) === String(oid));
    if (!found) { didAutoOpen.current = true; return; }
  ...
  return (
    <div className="p-8 space-y-8">
      <div className="flex items-start justify-between gap-4 flex-wrap">
        ...
        <button type="button" onClick={() => window.location.reload()} className="inline-flex items-center rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs font-semibold text-slate-900 hover:bg-slate-50">
          Rafraîchir
        </button>
      </div>
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
        <div className="flex items-center justify-between gap-3 flex-wrap">
          <div className="flex items-center gap-2 flex-wrap">
            {(["ALL", "NON_COMPLIANT", "IN_PROGRESS", "NOT_EVALUATED", "COMPLIANT"] as const).map((f) => {
              const labels: Record<string, string> = {
                ALL: `Tout (${totals.total})`,
                NON_COMPLIANT: `À risque (${totals.risky})`,
                IN_PROGRESS: `En cours (${totals.inProg})`,
                NOT_EVALUATED: `Non évaluées (${totals.notEval})`,
                COMPLIANT: `Stabilisées (${totals.stable})`,
              };
              const tones: Record<string, string> = {
                ALL: filter === f ? "border-slate-900 bg-slate-900 text-white" : "border-slate-200 bg-white text-slate-700 hover:bg-slate-50",
                NON_COMPLIANT: filter === f ? "border-rose-700 bg-rose-700 text-white" : "border-rose-200 bg-rose-50 text-rose-700 hover:bg-rose-100",
                IN_PROGRESS: filter === f ? "border-amber-700 bg-amber-700 text-white" : "border-amber-200 bg-amber-50 text-amber-700 hover:bg-amber-100",
                NOT_EVALUATED: filter === f ? "border-slate-900 bg-slate-900 text-white" : "border-slate-200 bg-slate-50 text-slate-700 hover:bg-slate-100",
                COMPLIANT: filter === f ? "border-emerald-700 bg-emerald-700 text-white" : "border-emerald-200 bg-emerald-50 text-emerald-700 hover:bg-emerald-100",
              };
              return (
                <button key={f} type="button" onClick={() => setFilter(f)} className={`rounded-full border px-3 py-1.5 text-xs font-semibold ${tones[f]}`}>
                  {labels[f]}
                </button>
              );
            })}
          ...
          <div className="w-full sm:w-96">
            <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Rechercher (titre / article / catégorie)…" className="w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-slate-900/10" />
          </div>
        </div>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
          {view.map((o) => {
            ...
            return (
              <button key={o.obligationId} type="button" onClick={() => openDrawer(o)} className={`text-left rounded-2xl border p-6 shadow-sm ${tone} hover:shadow-md transition focus:outline-none focus:ring-2 focus:ring-slate-900/10`}>
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    <div className="text-sm font-semibold uppercase tracking-widest text-slate-700 truncate">{o.legalRef ?? "—"}</div>
                  </div>
                  <span className={`inline-flex items-center rounded-full border px-2.5 py-1 text-[11px] font-semibold ${pillTone(status)}`}>{statusLabel(status)}</span>
                </div>
                ...
                <div className="mt-3 text-sm text-slate-600">
                  <span className="font-semibold text-slate-900">{o.useCasesCount}</span> cas d'usage • <span className="font-semibold text-slate-900">{o.systemsCount}</span> systèmes
                </div>
                <div className="mt-4">
                ...
                <div className="mt-4 flex items-center gap-2 flex-wrap">
                  <span className="inline-flex items-center rounded-full border border-rose-200 bg-rose-50 px-2.5 py-1 text-[11px] font-semibold text-rose-700">{o.nonCompliant} NC</span>
                  <span className="inline-flex items-center rounded-full border border-amber-200 bg-amber-50 px-2.5 py-1 text-[11px] font-semibold text-amber-700">{o.inProgress} EC</span>
                  <span className="inline-flex items-center rounded-full border border-slate-200 bg-slate-50 px-2.5 py-1 text-[11px] font-semibold text-slate-700">{o.notEvaluated} NE</span>
                  <span className="inline-flex items-center rounded-full border border-emerald-200 bg-emerald-50 px-2.5 py-1 text-[11px] font-semibold text-emerald-700">{o.compliant} OK</span>
                </div>
                <div className="mt-4 text-xs text-slate-600">Dernière activité : <span className="font-semibold">{prettyDate(o.lastUpdate)}</span></div>
              </button>
            );
          })}
        </div>
            ...
            <div className="border-b p-4 flex items-start justify-between gap-3">
              <div className="min-w-0">
                ...
                <div className="mt-1 text-xs text-slate-600">
                  {selected?.legalRef && <span className="font-semibold">{selected.legalRef}</span>}
                  {selected?.category && <span className="text-slate-500"> • </span>}
                  {selected?.category && <span className="font-semibold">{selected.category}</span>}
                </div>
              </div>
              <button type="button" onClick={closeDrawer} className="inline-flex items-center rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs font-semibold text-slate-900 hover:bg-slate-50">Fermer</button>
            </div>
            <div className="flex-1 overflow-auto p-4 space-y-4">
              <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                <div className="text-xs font-semibold text-slate-500">Lecture contrôleur</div>
                <div className="mt-1 text-sm font-semibold text-slate-900">
                  {selected ? <><span className="font-bold">{selected.useCasesCount}</span> cas d'usage impactés • <span className="font-bold">{selected.complianceRate}%</span> conforme</> : "—"}
                </div>
                <div className="mt-2 flex items-center gap-2 flex-wrap">
                  <span className="inline-flex items-center rounded-full border border-rose-200 bg-rose-50 px-2.5 py-1 text-[11px] font-semibold text-rose-700">{selected?.nonCompliant ?? 0} NC</span>
                  <span className="inline-flex items-center rounded-full border border-amber-200 bg-amber-50 px-2.5 py-1 text-[11px] font-semibold text-amber-700">{selected?.inProgress ?? 0} EC</span>
                  <span className="inline-flex items-center rounded-full border border-slate-200 bg-slate-50 px-2.5 py-1 text-[11px] font-semibold text-slate-700">{selected?.notEvaluated ?? 0} NE</span>
                  <span className="inline-flex items-center rounded-full border border-emerald-200 bg-emerald-50 px-2.5 py-1 text-[11px] font-semibold text-emerald-700">{selected?.compliant ?? 0} OK</span>
                </div>
              </div>
              <div>
              ...
              {drillErr && <div className="rounded-2xl border border-red-200 bg-red-50 p-3 text-sm text-red-700">{drillErr}</div>}
              {drillLoading ? (
                ...
                <div className="space-y-2">
                  {drillUseCases.map((u) => (
                    <Link key={u.stateId} href={`/dashboard/usecases/${encodeURIComponent(u.useCaseKey)}?openStateId=${encodeURIComponent(u.stateId)}`} className="block rounded-2xl border border-slate-200 bg-white p-4 hover:bg-slate-50 transition">
                      <div className="flex items-start justify-between gap-3">
                        <div className="min-w-0">
                          ...
                          <div className="mt-2 text-xs text-slate-500 flex items-center gap-2 flex-wrap">
                            <span>MAJ : <span className="font-semibold">{prettyDate(u.updatedAt)}</span></span>
                            {u.owner && <span>• Owner : <span className="font-semibold text-slate-900">{u.owner}</span></span>}
                            {u.dueDate && <span>• Deadline : <span className="font-semibold">{prettyDate(u.dueDate)}</span></span>}
                          </div>
                        </div>
